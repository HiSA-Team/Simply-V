...
The directory `simply_conf/` holds all the configuration related files, the main configuration flow is depicted here:
...
1. **Config checking**
    ...
    configuration flows) starts with [`mbus.init_configurations()`](simply_conf/buses/mbus.py#L42) that is the starting point for the creation of the
    tree hierarchy and for all the checks and sanitizations of the inputs.
    The configuration flows use the class hierarchy [`Parsers`](simply_conf/parsers) to parse and sanitize the input csv and the class hierarchy
    [`Factories`](simply_conf/factories) to centralize the objects creation.

2. **Software configuration flow (`config_sw`)**
   The entire software side is produced by the *config_sw* flow. In particular:
   - The software Makefile is generated by [`simplyv.update_sw_makefile()`](simply_conf/general/simplyv.py#L138).
   - The [Linker script](../sw/SoC/common/simplyv.ld) is generated from [`ld_template.py`](simply_conf/templates/ld_template.py).
   - The HAL header file is generated from [`halheader_template.py`](simply_conf/templates/halheader_template.py).
...
3. **Xilinx-related configuration**
   ...
   - [`simplyv.config_xilinx_makefile()`](simply_conf/general/simplyv.py#L181)
   - [`simplyv.config_xilinx_clock_domains()`](simply_conf/general/simplyv.py#L223)
   - [`simplyv.config_peripherals_ips()`](simply_conf/general/simplyv.py#L251)

4. **Bus-related configuration**
   All bus-related files (*config_bus* flow) are generated by:
   - [`crossbar_template.py`](simply_conf/templates/crossbar_template.py)
   - [`bus_interconnect_template.py`](simply_conf/templates/bus_interconnect_template.py)
   - [`clocks_template.py`](simply_conf/templates/clocks_template.py)

5. **Peripheral dumping**
   The generation of peripheral dumps for the *config_dump* flow is handled by [`dump_template.py`](simply_conf/templates/dump_template.py).
...
3. In one of the parser classes file, depending if the property is system o bus specific (NonLeaf or Leaf) add the new rules for parsing and validations, following the [`parser.py`](simply_conf/parsers/parser.py) design expressed in the header file.
...
The bash script [`docs.sh`](simply_conf/docs.sh) uses Pyreverse to automatically generate class diagrams
