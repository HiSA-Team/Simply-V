...
# "except Simply_V_Error" to capture all the custom Exception classes
class Simply_V_Error(Exception):
	pass

class Out_Of_Range_Error(Simply_V_Error):
	def __init__(self, param, value, min_val, max_val):
		message = f"{param} out of RANGE: {value} not in [{min_val}, {max_val}]"
		message += "[E_TYPE R]"
		super().__init__(message)

class Invalid_Type_Error(Simply_V_Error):
...
class Unsupported_Value_Error(Simply_V_Error):
...
class Conflict_Error(Simply_V_Error):
...
class Missing_Parameter_Error(Simply_V_Error):
...
class Naming_Convention_Error(Simply_V_Error):
